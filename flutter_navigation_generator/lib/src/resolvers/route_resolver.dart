...
const TypeChecker _flutterRouteAnnotationChecker = TypeChecker.typeNamed(FlutterRoute);
const TypeChecker _constructorAnnotationChecker = TypeChecker.typeNamed(FlutterRouteConstructor);
...
class RouteResolver {
  ...
  List<RouteConfig> resolve(ClassElement classElement) {
    final flutterRouteAnnotations = _flutterRouteAnnotationChecker.annotationsOf(
      classElement,
      throwOnUnresolved: false,
    );
    return flutterRouteAnnotations.map((annotation) => _resolveRoute(classElement, annotation)).toList();
  }

  ExecutableElement _resolveConstructorMethod(ClassElement classElement, String routeName) {
    ...
    final possibleFactoriesWithAnnotations = possibleFactories.asMap().map(
      (key, value) => MapEntry(value, _constructorAnnotationChecker.annotationsOf(value).map(ConstantReader.new)),
    );
    ...
    constructor =
        possibleFactoriesWithAnnotations.entries
            .firstWhereOrNull(
              (element) => element.value.any((annotation) => annotation.peek('routeName')?.stringValue == routeName),
            )
            ?.key;
    constructor ??=
        possibleFactoriesWithAnnotations.entries
            .firstWhereOrNull(
              (element) => element.value.any((annotation) => annotation.peek('routeName')?.stringValue == null),
            )
            ?.key;
  ...
  RouteConfig _resolveRoute(ClassElement classElement, DartObject flutterRouteAnnotation) {
    ...
    final routeName = routeNameValue ?? CaseUtil(classElement.displayName).kebabCase;
    ...
    final guards =
        flutterRoute
            ...
            .toList();
    final navigationType = NavigationType.values.firstWhere(
      (element) => element.index == flutterRoute.peek('navigationType')?.peek('index')?.intValue,
    );
    final includeQueryParametersIndex = flutterRoute.peek('includeQueryParameters')?.peek('index')?.intValue;
    final includeQueryParameters =
        includeQueryParametersIndex == null ? null : IncludeQueryParametersType.values[includeQueryParametersIndex];
